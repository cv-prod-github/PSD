...
.PARAMETER Upgrade
    ...
    In order to use this switch, upgrade an existing deployment share, the parameters psDeploymentFolder and <#
.SYNOPSIS
.DESCRIPTION
.PARAMETER psDeploymentFolder
    Specify the target MDT/PSD deployment share's physical folder location in the file system.
...
if (-not ($Upgrade.IsPresent)){
    ...
    # Create SMB-Share
    $Null = New-SmbShare -Name $psDeploymentShare -Path $psDeploymentFolder -FullAccess ([System.Security.Principal.SecurityIdentifier]::new($groupMapping["Administrators"])).Translate( [System.Security.Principal.NTAccount]).value -Description $description -ErrorAction Stop
...
if (!($Upgrade)) {
    ...
    $property = "$($script:PSDDrive):"Zuordnungen von Kontennamen und Sicherheitskennungen wurden nicht durchgeführt.
    ...
    icacls $psDeploymentFolder /grant """$(([System.Security.Principal.SecurityIdentifier]::new($groupMapping["Users"])).Translate( [System.Security.Principal.NTAccount]).value)"":(OI)(CI)(RX)" | Out-Null
    icacls $psDeploymentFolder /grant """$(([System.Security.Principal.SecurityIdentifier]::new($groupMapping["Administrators"])).Translate( [System.Security.Principal.NTAccount]).value)"":(OI)(CI)(F)" | Out-Null
    icacls $psDeploymentFolder /grant """$(([System.Security.Principal.SecurityIdentifier]::new($groupMapping["System"])).Translate( [System.Security.Principal.NTAccount]).value)"":(OI)(CI)(F)" | Out-Null

    Grant-SmbShareAccess -Name $psDeploymentShare -AccountName ([System.Security.Principal.SecurityIdentifier]::new($groupMapping["Everyone"])).Translate( [System.Security.Principal.NTAccount]).value -AccessRight Change -Force | Out-Null
    Revoke-SmbShareAccess -Name $psDeploymentShare -AccountName ([System.Security.Principal.SecurityIdentifier]::new($groupMapping["Owner"])).Translate( [System.Security.Principal.NTAccount]).value -Force | Out-Null
...
psDeploymentShare must have valid values.
    If this parameter is not used the script asumes that it is a new deployment.
.LINK
.NOTES
          FileName: Install.ps1
